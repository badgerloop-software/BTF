...
const int slaveAddress = 0x24; // <-- Your address of choice
...
void runSlave() {
    ...
    if (status >= 0)
    {
        ...
        while(1) {
            ...
            if(xfer.rxCnt > 1) {
                cout << "Received\n";
		memcpy(reg, xfer.rxBuf+1, 1);
		printf("put %x in reg\n", reg[0]);
            }
	    else if (xfer.rxCnt == 1) {
		bsc_xfer_t message;
		message.control = getControlBits(slaveAddress, true);
                cout << "Request to read\n";
                memcpy(message.txBuf, reg, 1);
		message.txCnt = 1;
		bscXfer(&message);
	    }
	    usleep(500);
